...
let fichasSelecionadas = new Set();
let fichaAtiva = null;
...
window.onload = () => {
  ...
  inicializarBancoAmeacasAutocomplete();
};

function inicializarBancoAmeacasAutocomplete() {
  const datalist = document.getElementById('dbAmeacaDatalist');
  const searchInput = document.getElementById('dbAmeacaSearch');
  if (!datalist || !searchInput) return;

  if (typeof AMEACAS_DB !== 'undefined' && Array.isArray(AMEACAS_DB)) {
    const nomesOrdenados = [...AMEACAS_DB].sort((a, b) => a.nome.localeCompare(b.nome));
    datalist.innerHTML = nomesOrdenados.map(a => `<option value="${a.nome}">${a.tipo || 'Criatura'} · ND ${a.nd || '?'}</option>`).join('');
    ...
    searchInput.addEventListener('input', () => {
      ...
      if (match) {
        const ameaca = {
          _id: 'am-' + Date.now() + Math.random().toString(36).substr(2, 5),
          _tipo: 'ameaca_t20',
          nome: match.nome,
          subtipo: `${match.tipo || 'Criatura'} · ND ${match.nd || '?'}`,
          rawData: match
        };
        bibliotecaAmeacas = bibliotecaAmeacas.filter(a => a.nome.toLowerCase() !== match.nome.toLowerCase());
        bibliotecaAmeacas.push(ameaca);
        localStorage.setItem('t20_biblioteca_ameacas', JSON.stringify(bibliotecaAmeacas));
        
        renderizarBibliotecaFichas();
        renderizar();
        
        searchInput.value = '';
        mostrarToast(`🩸 Ameaça "${match.nome}" importada com sucesso!`, 'sucesso');
      }
    });
  } else {
    console.warn("Banco de Ameaças (AMEACAS_DB) não foi carregado.");
  }
}
...
function carregarDados() {
  ...
  try {
    const ameacasSalvas = localStorage.getItem('t20_biblioteca_ameacas');
    if (ameacasSalvas) bibliotecaAmeacas = JSON.parse(ameacasSalvas);
  } catch (e) { bibliotecaAmeacas = []; }
  renderizarBibliotecaFichas();
  renderizar();
}
...
function removerFichaDaBiblioteca(nome) {
  fichasSelecionadas.delete('ficha::' + nome);
...
function renderizarBibliotecaFichas() {
  ...
  const totalItens = bibliotecaFichas.length + bibliotecaAmeacas.length;
  if (totalItens === 0) {
  ...
  if (bibliotecaFichas.length > 0) {
    html += `<div class="biblioteca-secao">
            <div class="biblioteca-secao-titulo">🧙 Fichas de Personagem</div>`;
    html += bibliotecaFichas.map(f => {
      const def = calcularDefesaFicha(f);
      const key = 'ficha::' + f.charName;
      const isChecked = fichasSelecionadas.has(key);
      const nomeSafe = f.charName.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
      return `
            <div class="ficha-badge ${isChecked ? 'ficha-selecionada' : ''}" style="margin-bottom:6px;">
                <label class="ficha-check-label" style="display: flex; align-items: center; width: 100%; cursor: pointer;">
                    <input type="checkbox" 
                           class="ficha-checkbox" 
                           ${isChecked ? 'checked' : ''} 
                           onchange="toggleFichaSelecionada('${key}')">
                ...
                <button onclick="removerFichaDaBiblioteca('${nomeSafe}')" class="btn-remove-ficha" title="Remover">✖</button>
            </div>`;
    }).join('');
    html += '</div>';
  }

  if (bibliotecaAmeacas.length > 0) {
    html += `<div class="biblioteca-secao">
            <div class="biblioteca-secao-titulo">⚔️ Ameaças</div>`;
    html += bibliotecaAmeacas.map(a => {
      const key = 'ameaca::' + a._id;
      const isChecked = fichasSelecionadas.has(key);
      const tipoIcon = a._tipo === 'ameaca_foundry' ? '🎲' : '🩸';
      return `
            <div class="ficha-badge ficha-badge-ameaca ${isChecked ? 'ficha-selecionada' : ''}" style="margin-bottom:6px;">
                <label class="ficha-check-label" style="display: flex; align-items: center; width: 100%; cursor: pointer;">
                    <input type="checkbox" 
                           class="ficha-checkbox" 
                           ${isChecked ? 'checked' : ''} 
                           onchange="toggleFichaSelecionada('${key}')">
                    <div class="ficha-badge-info">
                        <span class="ficha-nome">${tipoIcon} ${a.nome}</span>
                        <span class="ficha-detalhe">${a.subtipo}</span>
                    </div>
                </label>
                <button onclick="removerAmeacaDaBiblioteca('${a._id}')" class="btn-remove-ficha" title="Remover">✖</button>
            </div>`;
    }).join('');
    html += '</div>';
  }
...
function toggleFichaSelecionada(key) {
  if (fichasSelecionadas.has(key)) {
    fichasSelecionadas.delete(key);
  } else {
    fichasSelecionadas.add(key);
  }
  renderizarBibliotecaFichas();
  // Atualiza o texto dos botões nas cenas sem re-renderizar tudo
  const total = fichasSelecionadas.size;
  document.querySelectorAll('.btn-inserir-ficha').forEach(btn => {
    btn.textContent = total > 0
      ? `🧙 Colar Selecionadas (${total})`
      : '🧙 Colar Selecionadas';
  });
}
...
function colarSelecionadasNoPlot(cenaId) {
  if (fichasSelecionadas.size === 0) {
    mostrarToast('☑️ Selecione ao menos uma ficha ou ameaça na biblioteca.', 'aviso');
    return;
  }
  const cena = cenasAtuais.find(c => c.id === cenaId);
  if (!cena) return;
  ...
  fichasSelecionadas.forEach(key => {
    if (key.startsWith('ficha::')) {
      const nome = key.slice(7);
      const ficha = bibliotecaFichas.find(f => f.charName === nome);
      if (ficha) {
        fichaAtiva = ficha;
        textos.push(gerarResumoFicha());
        fichaAtiva = null;
      }
    } else if (key.startsWith('ameaca::')) {
      const id = key.slice(8);
      const ameaca = bibliotecaAmeacas.find(a => a._id === id);
      if (ameaca) {
        if (ameaca._tipo === 'ameaca_t20') textos.push(gerarResumoAmeacaT20(ameaca.rawData));
        else if (ameaca._tipo === 'ameaca_foundry') textos.push(gerarResumoAmeacaFoundry(ameaca.rawData));
      }
    }
  });
  ...
  mostrarToast(`📋 ${textos.length} item(ns) colado(s) no enredo!`, 'sucesso');
}

// ===== IMPORTAR AMEAÇAS =====

function importarAmeacaT20() {
  const input = document.createElement('input');
  input.type = 'file';
  input.accept = '.json,application/json';
  input.onchange = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        const dados = JSON.parse(ev.target.result);
        if (!dados.nome) throw new Error('Campo "nome" não encontrado. Verifique se é uma ficha de ameaça T20.');
        const ameaca = {
          _id: 'am-' + Date.now() + Math.random().toString(36).substr(2, 5),
          _tipo: 'ameaca_t20',
          nome: dados.nome,
          subtipo: `${dados.tipo || 'Criatura'} · ND ${dados.nd || '?'}`,
          rawData: dados
        };
        bibliotecaAmeacas = bibliotecaAmeacas.filter(a => a.nome !== dados.nome);
        bibliotecaAmeacas.push(ameaca);
        localStorage.setItem('t20_biblioteca_ameacas', JSON.stringify(bibliotecaAmeacas));
        renderizarBibliotecaFichas();
        renderizar();
        mostrarToast(`🩸 Ameaça "${dados.nome}" adicionada!`, 'sucesso');
      } catch (err) {
...
function importarAmeacaFoundry() {
  const input = document.createElement('input');
  input.type = 'file';
  input.accept = '.json,application/json';
  input.onchange = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        const dados = JSON.parse(ev.target.result);
        if (!dados.name || !dados.system) throw new Error('Formato Foundry inválido — verifique se exportou do Fichas de Nimb.');
        const nd = dados.system?.attributes?.nd || '?';
        const role = dados.system?.detalhes?.role || '';
        const ameaca = {
          _id: 'am-' + Date.now() + Math.random().toString(36).substr(2, 5),
          _tipo: 'ameaca_foundry',
          nome: dados.name,
          subtipo: `Foundry · ND ${nd}${role ? ' · ' + role : ''}`,
          rawData: dados
        };
        bibliotecaAmeacas = bibliotecaAmeacas.filter(a => a.nome !== dados.name || a._tipo !== 'ameaca_foundry');
        bibliotecaAmeacas.push(ameaca);
        localStorage.setItem('t20_biblioteca_ameacas', JSON.stringify(bibliotecaAmeacas));
        renderizarBibliotecaFichas();
        renderizar();
        mostrarToast(`🎲 Ameaça Foundry "${dados.name}" adicionada!`, 'sucesso');
      } catch (err) {
        mostrarToast('❌ Erro: ' + err.message, 'erro');
        console.error(err);
      }
...
function removerAmeacaDaBiblioteca(id) {
  fichasSelecionadas.delete('ameaca::' + id);
  bibliotecaAmeacas = bibliotecaAmeacas.filter(a => a._id !== id);
  localStorage.setItem('t20_biblioteca_ameacas', JSON.stringify(bibliotecaAmeacas));
  renderizarBibliotecaFichas();
  renderizar();
  mostrarToast('Ameaça removida.', 'aviso');
}

function gerarResumoAmeacaT20(a) {
  const fmt = (v) => { const n = parseFloat(v); return isNaN(n) ? (v || '?') : (n >= 0 ? '+' : '') + n; };
  const at = a.atributos || {};
  const attrStr = `FOR${fmt(at.for)} DES${fmt(at.des)} CON${fmt(at.con)} INT${fmt(at.int)} SAB${fmt(at.sab)} CAR${fmt(at.car)}`;
  const ataques = (a.ataques || []).map(at => {
    let linha = `${at.nome} ${at.bonus} · ${at.dano}`;
    if (at.desc) linha += ` (${at.desc.substring(0, 60)}${at.desc.length > 60 ? '...' : ''})`;
    return linha;
  }).join('\n  ');
  const habilidades = (a.habilidades || []).slice(0, 8).map(h => {
    const d = (h.desc || '').split('\n')[0];
    const resumo = d.length > 90 ? d.substring(0, 90) + '...' : d;
    return `• ${h.nome}${resumo ? ': ' + resumo : ''}`;
  }).join('\n  ');
  const pericias = (a.pericias || []).map(p => `${p.nome} ${p.valor}`).join(' | ');
  return `━━━ AMEAÇA: ${a.nome} (${a.tipo || 'Criatura'}) ━━━
ND: ${a.nd || '?'} | Iniciativa: ${a.iniciativa || '?'} | Percepção: ${a.percepcao || '?'}
PV: ${a.pv || '?'} | PM: ${a.pm || '?'} | Defesa: ${a.defesa || '?'} | Desl: ${a.desl || '?'}
Fort: ${a.fort || '?'} | Ref: ${a.ref || '?'} | Von: ${a.von || '?'}
Atributos: ${attrStr}
Ataques:
  ${ataques || 'Nenhum'}
Habilidades:
  ${habilidades || 'Nenhuma'}${pericias ? '\nPerícias: ' + pericias : ''}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
}

function gerarResumoAmeacaFoundry(a) {
  const sys = a.system || {};
  const attrs = sys.atributos || {};
  const fmtN = (v) => { const n = parseInt(v); return isNaN(n) ? '+0' : (n >= 0 ? '+' : '') + n; };
  const getAttrBonus = (key) => {
    const attr = attrs[key] || {};
    const total = (attr.value || 0) + (attr.bonus || 0) + (attr.base || 0) + (attr.racial || 0);
    return fmtN(total);
  };
  const attrStr = `FOR${getAttrBonus('for')} DES${getAttrBonus('des')} CON${getAttrBonus('con')} INT${getAttrBonus('int')} SAB${getAttrBonus('sab')} CAR${getAttrBonus('car')}`;
  const per = sys.pericias || {};
  const inic = fmtN((per.inic || {}).outros || 0);
  const fort = fmtN((per.fort || {}).outros || 0);
  const refl = fmtN((per.refl || {}).outros || 0);
  const vont = fmtN((per.vont || {}).outros || 0);
  const atr = sys.attributes || {};
  const pv = (atr.pv || {}).max || 0;
  const pm = (atr.pm || {}).max || 0;
  const defesa = (atr.defesa || {}).base || 10;
  const nd = atr.nd || '?';
  const desl = (atr.movement || {}).walk || 0;
  const det = sys.detalhes || {};
  const equip = det.equipamento || '';
  const tesouro = det.tesouro || '';
  const role = det.role || '';
  const habilidades = (a.items || []).slice(0, 8).map(item => {
    const desc = (item.system?.description?.value || '').replace(/<[^>]+>/g, '').trim();
    const resumo = desc.length > 90 ? desc.substring(0, 90) + '...' : desc;
    return `• ${item.name}${resumo ? ': ' + resumo : ''}`;
  }).join('\n  ');
  return `━━━ AMEAÇA: ${a.name}${role ? ' (' + role + ')' : ''} ━━━
ND: ${nd} | Iniciativa: ${inic} | Desl: ${desl}m
PV: ${pv} | PM: ${pm} | Defesa: ${defesa}
Fort: ${fort} | Ref: ${refl} | Von: ${vont}
Atributos: ${attrStr}${equip ? '\nEquipamento: ' + equip : ''}${tesouro ? ' | Tesouro: ' + tesouro : ''}
Habilidades:
  ${habilidades || 'Nenhuma'}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
}
